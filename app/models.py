...
class Errata(models.Model):
    ...
    internal_notes = models.TextField(blank=True, null=True, help_text='Only users with app admin access can view and edit the contents of this field.')
    ...
    file_1 = models.FileField(upload_to='app/user_uploads/1/', blank=True, null=True)
    file_2 = models.FileField(upload_to='app/user_uploads/2/', blank=True, null=True)
    ...
    def clean(self):
        ...
        if (self.status == 'Editorial Review' or self.status == 'Reviewed' or self.status == 'Completed') and not self.is_assessment_errata:
            raise ValidationError({'is_assessment_errata': 'You must specify if this is an assessment app.'})
