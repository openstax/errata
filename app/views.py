...
class ErrataFilter(FilterSet):
    ...
    class Meta:
        model = Errata
        fields = ['openstax_book', 'archived', 'is_assessment_errata', 'is_assessment_errata__not']
...
@permission_required('app..erratum.can_change_errata', login_url="/admin/login/?next=/app/dashboard/")
def dashboard(request):
    ...
    return render(request, 'app/dashboard.html', context={'app': errata,
...
@permission_required('app..erratum.can_change_errata', login_url="/admin/login/?next=/app/list/")
def list(request):
    ...
    return render(request, 'app/list.html', {'app': errata,
                                                'status': status,
                                                'book': book,
                                                'type': type})


@permission_required('app..erratum.can_change_errata', login_url="/admin/login/?next=/app/edit/")
def edit(request):
    edit_errata = request.GET.get('app', False)
    errata = False
    user = None
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if request.POST.get('duplicate', False):
                ...
                return redirect('/apps/cms/api/app/admin/dashboard/') #TODO: change to URL resolver name
    ...
    return render(request, 'app/edit.html', {'app': errata,
                                                'form': form,
                                                'user': user})
...
def duplicate(errata):
    errata.pk = None
    errata.save()
    return redirect('/apps/cms/api/app/admin/edit/{}'.format(errata.pk)) #TODO: Change to use url resolver name